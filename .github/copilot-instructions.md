...
- `webapp/` contains the Vue 3 + Vite configuration UI used to set WiFi credentials, API key, and display settings for the device.
...
- The web interface is a Vue single-page app under `webapp/src/` with views, shared components, router setup, and a small global store.
...
3. When changing settings, routes, payloads, or validation rules, verify the firmware handlers and the Vue UI still match.
4. Keep the webapp lightweight and compatible with the current Vue 3 + Vite stack already in the repo.
